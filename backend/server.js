...
mongoose.connect("mongodb+srv://alayna2k23:XLrO7pHUEKiQREmq@cluster0.2ygko3y.mongodb.net/alayna?retryWrites=true&w=majority",{
...
let protect = async (req,res ,next) =>{
    ...
    if(req.headers.authorization && req.headers.authorization.startsWith("Classic")){
...
try{
    ...
    if(!reciver){
        res.send({message:"Reciver not found..."})
    }
...
// console.log(reciver.points)
        ...
        console.log(reciver)
        await reciver.save();
        res.send({message:"Loyalty points share successfully"})
    }else{
    ...
    if(em){
        ...
        ema.save(err =>{
            if(err){
                ...
                res.status(200).send({code:200,message:"THANKYOU !!! you are subscribed"})
            }
        })
    }
}catch(err){
